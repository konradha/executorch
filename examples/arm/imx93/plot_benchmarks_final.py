...
def plot_all(records, output_dir):
    ...
    for i, value in enumerate(ms):
        ax.text(i, value + 1, f"{value:.1f} ms", ha="center", va="bottom", fontsize=9)
    ...
    ax.bar(models, cosines, color=colors, width=0.6)
    ...
    for p, c, m in zip(params, cycles, models, strict=True):
        ax.annotate(
            m, (p, c / 1e6), textcoords="offset points", xytext=(8, 4), fontsize=9
        )
    ...
    ax.set_title(
        "i.MX93 Ethos-U65-256 Fastpath Results", pad=20, fontsize=12, weight="bold"
    )
