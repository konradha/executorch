...
for i, (m, v) in enumerate(zip(models, ms)):
    if v:
        ...
        ax1.text(2, i, "IOCTL failure (NXP firmware)", va="center", fontsize=9,
...
for i, (v, s) in enumerate(zip(cosine, status)):
    if v:
        ax2.text(v + 0.001, i, f"{v:.3f}", va="center", fontsize=10)
    else:
        ax2.text(0.15, i, "broken", va="center", fontsize=9, color="#b2182b", style="italic")
...
def sort_key(op):
    recs = matrix.get(op, {})
    cos_vals = [recs[s].get("cosine", 0) for s in sizes if s in recs and recs[s].get("status") == "ok"]
    if cos_vals:
...
for row, op in enumerate(sorted_ops):
    ...
    for col, sz in enumerate(sizes):
        ...
        rect = mpatches.FancyBboxPatch(
            (x, y), cell_w, cell_h, boxstyle="round,pad=0.05",
...
for col, sz in enumerate(sizes):
    x = col * (cell_w + gap)
    ax.text(x + cell_w / 2, len(sorted_ops) + 0.1, f"size {sz}",
            ha="center", va="bottom", fontsize=11)

ax.set_xlim(-0.2, len(sizes) * (cell_w + gap))
ax.set_ylim(-0.3, len(sorted_ops) + 0.6)
ax.axis("off")
ax.set_title("green = correct   orange = wrong   gray = not delegated",
             fontsize=10, pad=12, color="#555")
